...
enemy_x_pos = int((screen_width - enemy_width) / 2)    # Enemy 캐릭터의 시작 가로 위치
enemy_y_pos = screen_height/2   # Enemy 캐릭터의 시작 세로 위치
...
while isRunning:  # 게임이 진행되는 동안에
    ...
    character_rect.top = character_y_pos    # 현재 캐릭터 위치(화면상에 그려지는_변화 반영)
    # Enemy
    enemy_rect = enemy.get_rect()
    enemy_rect.left = enemy_x_pos
    enemy_rect.top = enemy_y_pos            # 현재 Enemy 캐릭터 위치(화면상에 그려지는_변화 반영)
